...
/** Markers with special meaning. */
enum PredefinedInsertMarker {
    Undefined = 0,
    ...
    /** Marker for any future whitespace to be ignored. */
    WhitespaceEraser = -102,
    Error = -103,

    // Block markers.

    /** Indirect index into the range list. */
    Range = -100000,

    /** Indirect index into the alignment groups list. */
    Alignment = -200000,

    /** Indirect index into the whitespace list. */
    WhitespaceBlock = -300000,
}
...
export class GrammarFormatter {
    ...
    // When scanning a range this contains the start token index.
    private currentRangeIndex: InsertMarker;

    // Holds the start index of a range when collecting.
    ...
    public formatGrammar(options: IFormattingOptions, start?: number, stop?: number): [string, number, number] {
        ...
        this.ranges = [];
        this.currentRangeIndex = PredefinedInsertMarker.Undefined;
        this.rangeStart = -1;
        ...
        for (let i = startIndex; i <= endIndex; ++i) {
            ...
            if (token.type !== ANTLRv4Lexer.WS && this.lastEntryIs(PredefinedInsertMarker.WhitespaceEraser)) {
            ...
            switch (token.type) {
                case ANTLRv4Lexer.WS: {
                    ...
                    if (this.lastEntryIs(PredefinedInsertMarker.WhitespaceEraser)) {
                    ...
                    if (!localCommentAhead || !hasLineBreaks) {
                        if (!hasLineBreaks || coalesceWhitespaces || this.singleLineBlockNesting > 0) {
                            if (!this.lastEntryIs(PredefinedInsertMarker.Whitespace)) {
                                this.addSpace();
                            }
                            break;
                        }
                    ...
                    if (localCommentAhead && this.lastCodeTokenIs(ANTLRv4Lexer.LPAREN)
                        ...
                        while (j >= 0) {
                            if (this.entryIs(j, PredefinedInsertMarker.LineBreak)) {
                                ++breakCount;
                            } else {
                                break;
                            }
                    ...
                    const lineBreaks: number[] = Array(breakCount).fill(PredefinedInsertMarker.LineBreak);
                ...
                case ANTLRv4Lexer.DOC_COMMENT: {
                    ...
                    if (this.options.reflowComments && comment.indexOf("\n") > 0) {
                        ...
                        const whitespaceIndex = PredefinedInsertMarker.WhitespaceBlock - this.whitespaceList.length;
                ...
                case ANTLRv4Lexer.OR: {
                    ...
                    if (this.singleLineBlockNesting > 1) {
                        ...
                        if (!inSingleLineRule) {
                            ...
                            if (this.outputPipeline.length > 0 && !this.lastEntryIs(PredefinedInsertMarker.LineBreak)) {
                                this.addLineBreak();
                            }
                            this.pushCurrentIndentation();
                ...
                case ANTLRv4Lexer.RARROW: {
                    ...
                    if (this.options.alignTrailers) {
                        ...
                        if (!this.lastEntryIs(PredefinedInsertMarker.Space)) {
                            this.addSpace();
                        }
                    }
                    this.add(i);
                ...
                case ANTLRv4Lexer.POUND: {
                    ...
                    if (!willUseAlignment && !this.lastEntryIs(PredefinedInsertMarker.Space)) {
                        this.addSpace();
                    }
                    this.add(i);
                    this.addSpace();
        ...
        if (this.tokens[endIndex].type !== ANTLRv4Lexer.WS) {
            ...
            if (this.lastEntryIs(PredefinedInsertMarker.Alignment)) {
                this.removeLastEntry();
            }
            this.removeTrailingWhitespaces();
        }
        ...
        for (const entry of this.outputPipeline) {
            switch (entry) {
                case PredefinedInsertMarker.LineBreak: {
                ...
                default:
                    if (entry < 0) {
                        ...
                        if (this.isWhitespaceBlock(entry)) {
                            result += this.whitespaceList[-(entry - PredefinedInsertMarker.WhitespaceBlock)];
    ...
    private entryIs(index: number, marker: InsertMarker): boolean {
        ...
        switch (marker) {
            case PredefinedInsertMarker.Whitespace: {
                return entry === PredefinedInsertMarker.LineBreak || entry === PredefinedInsertMarker.Space
                    || entry === PredefinedInsertMarker.Tab;
            }

            case PredefinedInsertMarker.Space: {
                return entry === PredefinedInsertMarker.Space;
            }

            case PredefinedInsertMarker.Tab: {
                return entry === PredefinedInsertMarker.Tab;
            }

            case PredefinedInsertMarker.LineBreak: {
                return entry === PredefinedInsertMarker.LineBreak;
            }

            case PredefinedInsertMarker.Comment: {
                if (entry < 0) {
                    return false;
                }
    ...
    private lineHasLeadingNonWhitespaceContent(): boolean {
        ...
        while (--index > 0) {
            if (this.outputPipeline[index] !== PredefinedInsertMarker.Space
                && this.outputPipeline[index] !== PredefinedInsertMarker.Tab) {
                break;
            }
        }
        if (index <= 0) {
        ...
        return this.outputPipeline[index] !== PredefinedInsertMarker.LineBreak;
    }
    ...
    private lastCodeTokenIs(marker: InsertMarker): boolean {
        ...
        while (i >= 0) {
            if (!this.entryIs(i, PredefinedInsertMarker.WhitespaceEraser)
                ...
                && !this.entryIs(i, PredefinedInsertMarker.Comment)) {
                break;
            }
            --i;
        }
    ...
    private removeLastEntry() {
        ...
        this.outputPipeline.pop();
        switch (lastEntry) {
            case PredefinedInsertMarker.WhitespaceEraser: {
                break; // Ignore.
            }
    ...
    private removeTrailingTabsAndSpaces() {
        ...
        while (this.lastEntryIs(PredefinedInsertMarker.Space) || this.lastEntryIs(PredefinedInsertMarker.Tab)) {
            this.removeLastEntry();
        }
    }
    ...
    private pushCurrentIndentation(force = false) {
        ...
        if (this.options.useTab) {
            const tabs: number[] = Array(this.currentIndentation).fill(PredefinedInsertMarker.Tab);
            ...
            const spaces: number[] = Array(this.currentIndentation * (this.options.indentWidth ?? 4))
                .fill(PredefinedInsertMarker.Space);
    ...
    private applyLineContinuation() {
        ...
        if (!this.lastEntryIs(PredefinedInsertMarker.LineBreak)) {
            this.outputPipeline.push(PredefinedInsertMarker.LineBreak);
            ++this.currentLine;
        }
        this.currentColumn = 0;
        this.pushCurrentIndentation(true);
        if (this.options.useTab) {
            this.outputPipeline.push(PredefinedInsertMarker.Tab);
        } else {
            const spaces: number[] = Array(this.options.continuationIndentWidth).fill(PredefinedInsertMarker.Space);
            this.outputPipeline.push(...spaces);
        }
    ...
    private add(marker: InsertMarker) {
        ...
        switch (marker) {
            case PredefinedInsertMarker.WhitespaceEraser: { // Doesn't move current position.
                this.outputPipeline.push(marker);
    ...
    private addRaw(start: InsertMarker, stop: InsertMarker): void {
        ...
        this.outputPipeline.push(PredefinedInsertMarker.Range - this.currentRangeIndex++);
    }

    private addSpace() {
        if (this.outputPipeline.length > 0
            && !this.lastEntryIs(PredefinedInsertMarker.Space)
            && !this.lastEntryIs(ANTLRv4Lexer.LINE_COMMENT)) {
            this.add(PredefinedInsertMarker.Space);
        }
    }
    ...
    private addLineBreak(force = false) {
        if (this.singleLineBlockNesting === 0 || force) {
            ...
            this.add(PredefinedInsertMarker.LineBreak);
        }
    }
    ...
    private ensureMinEmptyLines(): void {
        ...
        if (this.options.minEmptyLines! > 0) {
            ...
            for (let i = this.outputPipeline.length - 1; i > 0 && lineBreakCount > 0; --i) {
                if (this.entryIs(i, PredefinedInsertMarker.LineBreak)) {
                    --lineBreakCount;
                } else if (!this.entryIs(i, PredefinedInsertMarker.Whitespace)) {
                    break;
                }
            }

            const lineBreaks: number[] = Array(lineBreakCount).fill(PredefinedInsertMarker.LineBreak);
            this.outputPipeline.push(...lineBreaks);
            this.currentLine += lineBreakCount;
            if (lineBreakCount > 0) {
                this.currentColumn = 0;
            }
        } else if (!this.lastEntryIs(PredefinedInsertMarker.LineBreak)) {
            this.addLineBreak();
        }
    }
                    ...
                    switch (groups[1]) {
                        ...
                        case "alignTrailers": {
                            if ((
                                ...
                                this.add(PredefinedInsertMarker.Error);
                            }
                            break;
                        }
    ...
    private addAlignmentEntry(type: AlignmentType): void {
        ...
        if (status.lastLine !== this.currentLine) {
            ...
            this.outputPipeline.push(PredefinedInsertMarker.Alignment);
                ...
                for (const group of alignment.groups) {
                    ...
                    for (const member of group) {
                        ...
                        if (member < this.outputPipeline.length) {
                            console.assert(this.outputPipeline[member] <= PredefinedInsertMarker.Alignment);
    ...
    private columnForEntry(offset: number): number {
        ...
        let run = offset;
        while (--run > -1) {
            if (this.outputPipeline[run] === PredefinedInsertMarker.LineBreak) {
                break;
            }
        }
    ...
    private isRangeBlock = (marker: InsertMarker): boolean => {
        return (marker <= PredefinedInsertMarker.Range) && (marker > PredefinedInsertMarker.Alignment);
    };
    ...
    private isWhitespaceBlock = (marker: InsertMarker): boolean => {
        return (marker <= PredefinedInsertMarker.WhitespaceBlock);
    };
}
